    ...
    try {
        if (!sub) {
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: `🌸 *Uso:* .animu <tipo>\n\n💫 *Tipos disponíveis:*\n${apiTypes.split(', ').map(type => `• ${type}`).join('\n')}\n\n*Exemplo:* .animu kiss` }, { quoted: message });
            } catch {
        ...
        if (!supported.includes(sub)) {
            await sock.sendMessage(chatId, { text: `❌ Tipo não suportado: ${sub}\n\n🌸 *Tente um destes:*\n${supported.map(type => `• ${type}`).join('\n')}` }, { quoted: message });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: '❌ Erro ao buscar anime. Tente novamente! 🌸' }, { quoted: message });
    }
}
