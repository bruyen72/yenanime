...
async function updateCommand(sock, chatId, message, senderIsSudo, zipOverride) {
    if (!message.key.fromMe && !senderIsSudo) {
        await sock.sendMessage(chatId, { text: '🔒 *Acesso Negado!*\n\n👑 *Apenas o dono do bot ou usuários sudo podem usar .update*\n\n⚠️ Este comando é restrito para administradores do sistema\n\n✨ *Yen-Bot* - Segurança máxima! 🌸' }, { quoted: message });
        return;
    }
    try {
        // Minimal UX
        ...
        try {
            ...
            await sock.sendMessage(chatId, { text: '✅ Reiniciado com sucesso\n Digite .ping para verificar a versão mais recente.' }, { quoted: message });
        }
