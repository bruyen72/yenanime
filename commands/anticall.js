...
async function anticallCommand(sock, chatId, message, args) {
    ...
    if (sub === 'status') {
        await sock.sendMessage(chatId, { text: `Anticall está atualmente *${state.enabled ? 'LIGADO' : 'DESLIGADO'}*.` }, { quoted: message });
        return;
    }

    const enable = sub === 'on';
    writeState(enable);
    await sock.sendMessage(chatId, { text: `Anticall agora está *${enable ? 'ATIVADO' : 'DESATIVADO'}*.` }, { quoted: message });
}
